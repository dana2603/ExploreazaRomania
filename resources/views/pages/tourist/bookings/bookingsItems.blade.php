                ...
                <div class="row">
                    <div class="col-6">
                        @if($booking->status == "request")
                            <p class="heading">Status: <span class="booking-detail status request">Cerere trimisă/în curs de aprobare</span></p>
                        @elseif($booking->status == "pay")
                            <p class="heading">Status: <button class="btn button-primary-inverted-no-border booking-detail status pay" data-booking-id="{{$booking->id}}">Plătește aici</button></p>
                        @elseif($booking->status == "paid")
                        <p class="heading">Status: <span class="booking-detail status paid">Sejur plătit</span></p>
                        @elseif($booking->status == "ongoing")
                            <p class="heading">Status: <span class="booking-detail status ongoing">Sejur acceptat/în curs de desfășurare</span></p>
                        @elseif($booking->status == "complete")
                            <p class="heading">Status: <span class="booking-detail status complete">sejur/prenotare completată</span></p>
                        @elseif($booking->status == "rejected")
                            <p class="heading">Status: <span class="booking-detail status rejected">cerere prenotare sejur respinsă pe data de: {{date('d-m-Y', strtotime($booking->updated))}}</span></p>
                        @endif
                    </div>
                    <div class="col-6">
